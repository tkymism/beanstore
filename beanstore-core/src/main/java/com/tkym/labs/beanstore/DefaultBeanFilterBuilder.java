...
import com.tkym.labs.beanstore.api.BeanFilterFactory;
...
public class DefaultBeanFilterBuilder<BT, KT, PT> implements BeanFilterBuilder<BT, KT, PT>{
	...
	private final BeanFilterFactory<BT, PT> factory;
	public DefaultBeanFilterBuilder(BeanQuery<BT, KT> query, PropertyMeta<BT, PT> propertyMeta){
		this.query = query;
		factory = BeanQueryUtils.filter(propertyMeta);
	}
	@Override
	public BeanQuery<BT, KT> equalsTo(PT value) {
		return query.filter(factory.equalsTo(value));
	}
	@Override
	public BeanQuery<BT, KT> greaterThan(PT value) {
		return query.filter(factory.greaterThan(value));
	}
	@Override
	public BeanQuery<BT, KT> greaterEqual(PT value) {
		return query.filter(factory.greaterEqual(value));
	}
	@Override
	public BeanQuery<BT, KT> lessThan(PT value) {
		return query.filter(factory.lessThan(value));
	}
	@Override
	public BeanQuery<BT, KT> lessEqual(PT value) {
		return query.filter(factory.lessEqual(value));
	}
	@Override
	public BeanQuery<BT, KT> notEquals(PT value) {
		return query.filter(factory.notEquals(value));
	}
	@Override
	public BeanQuery<BT, KT> in(PT... value) {
		return query.filter(factory.in(value));
	}
	@Override
	public BeanQuery<BT, KT> startsWith(PT value) {
		return query.filter(factory.startsWith(value));
	}
	@Override
	public BeanQuery<BT, KT> endsWith(PT value) {
		return query.filter(factory.endsWith(value));
	}
	@Override
	public BeanQuery<BT, KT> contains(PT value) {
		return query.filter(factory.contains(value));
	}
}