...
import static com.tkym.labs.beanstore.BeanstoreServiceInterpreter.target;
...
public class BeanstoreServiceInterpreterTest {
	...
	public void testFilterCase001(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				processFilter(p(PERSON.name).equalsTo("a00001"));
	...
	public void testFilterCase002(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				processFilter(p(PERSON.name).lessThan("a00001"));
		assertThat(r.size(), is(1));
	}
	...
	public void testFilterCase003(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				processFilter(p(PERSON.name).lessEqual("a00001"));
	...
	public void testFilterCase004(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				processFilter(p(PERSON.name).greaterThan("a00001"));
	...
	public void testFilterCase005(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				processFilter(p(PERSON.name).greaterEqual("a00001"));
	...
	public void testFilterCase006(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				processFilter(p(PERSON.name).notEquals("a00001"));
		assertThat(r.size(), is(99));
	}
	...
	public void testFilterCase007(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				processFilter(p(PERSON.name).in("a00001", "a00002", "a00003"));
	...
	public void testFilterCase008(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				processFilter(p(PERSON.name).startsWith("a00"));
	...
	public void testFilterCase009(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				processFilter(p(PERSON.name).startsWith("b00"));
	...
	public void testFilterCase010(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				processFilter(p(PERSON.name).endsWith("001"));
		assertThat(r.size(), is(1));
	}
	...
	public void testFilterCase011(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				processFilter(p(PERSON.name).contains("00000"));
		assertThat(r.size(), is(1));
	}
	...
	public void testFilterCase101(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				processFilter(p(ACCOUNT.address).equalsTo("00003st.001"));
		assertThat(r.size(), is(1));
	}
	...
	public void testFilterCase102(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				processFilter(p(ACCOUNT.address).lessThan("00003st.001"));
	...
	public void testFilterCase103(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				processFilter(p(ACCOUNT.address).lessEqual("00003st.001"));
	...
	public void testFilterCase104(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				processFilter(p(ACCOUNT.address).greaterThan("00003st.001"));
	...
	public void testFilterCase105(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				processFilter(p(ACCOUNT.address).greaterEqual("00003st.001"));
	...
	public void testFilterCase106(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				processFilter(p(ACCOUNT.address).notEquals("00003st.001"));
	...
	public void testFilterCase107(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				processFilter(p(ACCOUNT.address).in("00003st.001", "00001st.003", "00002st.003"));
		assertThat(r.size(), is(3));
	}
	...
	public void testFilterCase108(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				processFilter(p(ACCOUNT.address).startsWith("0000"));
		assertThat(r.size(), is(100));
	}
	...
	public void testFilterCase109(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				processFilter(p(ACCOUNT.address).endsWith("1st.001"));
	...
	public void testFilterCase110(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				processFilter(p(ACCOUNT.address).contains("1st.0"));
		assertThat(r.size(), is(100));
	}
	...
	public void testCriteriaCase001(){
		Set<Key<Person, Long>> r = 
			target(REPO.get(PERSON).queryAll()).
			filter(p(PERSON.name).equalsTo("a00001"));
		assertThat(r.size(), is(1));
	}
	...
	public void testCriteriaCase002(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				filter(p(PERSON.name).lessThan("a00001"));
		assertThat(r.size(), is(1));
	}
	...
	public void testCriteriaCase003(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				filter(p(PERSON.name).lessEqual("a00001"));
		assertThat(r.size(), is(2));
	}
	...
	public void testCriteriaCase004(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				filter(p(PERSON.name).greaterThan("a00001"));
		assertThat(r.size(), is(98));
	}
	...
	public void testCriteriaCase005(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				filter(p(PERSON.name).greaterEqual("a00001"));
		assertThat(r.size(), is(99));
	}
	...
	public void testCriteriaCase006(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				filter(p(PERSON.name).notEquals("a00001"));
		assertThat(r.size(), is(99));
	}
	...
	public void testCriteriaCase007(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				filter(p(PERSON.name).in("a00001", "a00002", "a00003"));
		assertThat(r.size(), is(3));
	}
	...
	public void testCriteriaCase008(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				filter(p(PERSON.name).startsWith("a00"));
		assertThat(r.size(), is(100));
	}
	...
	public void testCriteriaCase009(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				filter(p(PERSON.name).startsWith("b00"));
		assertThat(r.size(), is(0));
	}
	...
	public void testCriteriaCase010(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				filter(p(PERSON.name).endsWith("001"));
		assertThat(r.size(), is(1));
	}
	...
	public void testCriteriaCase011(){
		Set<Key<Person, Long>> r = 
				target(REPO.get(PERSON).queryAll()).
				filter(p(PERSON.name).contains("00000"));
		assertThat(r.size(), is(1));
	}
	...
	public void testCriteriaCase101(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				filter(p(ACCOUNT.address).equalsTo("00003st.001"));
		assertThat(r.size(), is(1));
	}
	...
	public void testCriteriaCase102(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				filter(p(ACCOUNT.address).lessThan("00003st.001"));
		assertThat(r.size(), is(31));
	}
	...
	public void testCriteriaCase103(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				filter(p(ACCOUNT.address).lessEqual("00003st.001"));
		assertThat(r.size(), is(32));
	}
	...
	public void testCriteriaCase104(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				filter(p(ACCOUNT.address).greaterThan("00003st.001"));
		assertThat(r.size(), is(1000-32));
	}
	...
	public void testCriteriaCase105(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				filter(p(ACCOUNT.address).greaterEqual("00003st.001"));
		assertThat(r.size(), is(1000-31));
	}
	...
	public void testCriteriaCase106(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				filter(p(ACCOUNT.address).notEquals("00003st.001"));
		assertThat(r.size(), is(999));
	}
	...
	public void testCriteriaCase107(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				filter(p(ACCOUNT.address).in("00003st.001", "00001st.003", "00002st.003"));
		assertThat(r.size(), is(3));
	}
	...
	public void testCriteriaCase108(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				filter(p(ACCOUNT.address).startsWith("0000"));
		assertThat(r.size(), is(100));
	}
	...
	public void testCriteriaCase109(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				filter(p(ACCOUNT.address).endsWith("1st.001"));
		assertThat(r.size(), is(10));
	}
	...
	public void testCriteriaCase110(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).
				filter(p(ACCOUNT.address).contains("1st.0"));
		assertThat(r.size(), is(100));
	}
	...
	public void testCompositeCase001(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).filter(
					and(
						p(ACCOUNT.address).contains("st.0"),
						p(ACCOUNT.address).startsWith("0000"),
						p(ACCOUNT.address).endsWith("1")
					)
				);
		assertThat(r.size(), is(10));
	}
	...
	public void testCompositeCase002(){
		Set<Key<Account, String>> r = 
				target(REPO.get(ACCOUNT).queryAll()).filter(
					or(
						p(ACCOUNT.address).contains("st.001"),
						p(ACCOUNT.address).endsWith("st.002"),
						p(ACCOUNT.address).startsWith("00001")
					)
				);
